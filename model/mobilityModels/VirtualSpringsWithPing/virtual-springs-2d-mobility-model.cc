...
void 
VirtualSprings2dMobilityModel::ReceivePacket (Ptr<Socket> socket)
{
  while (socket->Recv ())
  {
...
{
  m_prevVel = Vector(0,0,0);
  m_socket -> SetRecvCallback (MakeCallback (&ns3::VirtualSprings2dMobilityModel::ReceivePacket, this));
...
VirtualSprings2dMobilityModel::DoInitializePrivate (void)
{
  ...
  m_socket -> Send (Create<Packet> (56));

  Simulator::Schedule(m_modeTime, &VirtualSprings2dMobilityModel::DoTryMove, this);
  
}

void
VirtualSprings2dMobilityModel::DoTryMove (void)
{
  if (m_ping)
  {
    ...
    if (force.GetLength () > m_tol)
    {
      ...
      m_prevVel = vector; //Save velocity vector to go back if needed
      m_helper.Unpause();
    }
    else 
    {
  ...
  { 
    // Vector newVel;
    // newVel.x = -m_prevVel.x;
    // newVel.y = -m_prevVel.y;
    // newVel.z = -m_prevVel.z;
...
void 
VirtualSprings2dMobilityModel::AddSocket(Ptr<Socket> socket)
{
  m_socket = socket;
}
