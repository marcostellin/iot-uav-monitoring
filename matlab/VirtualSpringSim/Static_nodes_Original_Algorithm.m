...
N_UAVS = 6;     %Number of Aerial vehicles
...
for i=(1:DELTA:TIME)
    ...
    for j = (1:N_UAVS)
        for k = (1:N_UAVS)
            if AtAConn(j,k) == 1
                lbLink = friisPathLoss(TX_POWER_ATA,AtADists(j,k),WIFI_F) - LB_TH_ATA;
                disp = nthroot(max(lbLink, LB_REQ_ATA)/min(lbLink,LB_REQ_ATA),ALFA)-1; 
    ...
    for j = (1:N_UAVS)
        for k = (1:N_EUS)
            ...
            if d < RANGE_ATG
                lbLink = logPathLoss(TX_POWER_ATG,REF_DIST,REF_LOSS,PATH_EXP,threeDistance([UAVsCurPos(j,:) 100] , [EUsCurPos(k,:) 100])) - LB_TH_ATG;
                disp = nthroot(max(lbLink, LB_REQ_ATG)/min(lbLink,LB_REQ_ATG),1)-1; 
    ...
    hold on;
    plot(50, 0, 'c*');
    plot(EUsCurPos(:,1)/10, EUsCurPos(:,2)/10, 'r*');
